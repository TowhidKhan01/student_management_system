...
class FeeAmountController extends Controller
{
    ...
    public function StoreFeeAmount(Request $request)
    {
        ...
        $notification = array(
            'message' => 'Fee amount Inserted Succesfully',
            'alert-type' => 'info'
    ...
    public function UpdateFeeAmount(Request $request, $fee_category_id)
    {
        if ($request->class_id == NULL) {
            $notification = array(
                'message' => 'Sorry you did not  selected any class amount',
                'alert-type' => 'error'
            ...
            $countClass = count($request->class_id);
            FeeCategoryAmount::where('fee_catgory_id', $fee_category_id)->delete();
            for ($i = 0; $i < $countClass; $i++) {
                $fee_amount = new FeeCategoryAmount();
                $fee_amount->fee_category_id = $request->fee_category_id;
                $fee_amount->class_id = $request->class_id[$i];
